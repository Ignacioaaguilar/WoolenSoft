Imports Controlador
Public Class FacturacionProveedores
    ...
    Public FacturacionProveedor_Enc_estructura(0) As Controlador.FacturacionProveedor.eEncabezadoFacturaProveedor
    Public FacturacionProveedor_Cuerpo_estructura(0) As Controlador.FacturacionProveedor.eCuerpoFacturaProveedor
        ...
        Try
            ...
            ElseIf txtCodigoProveedor.Text <> "" Then
                ...
                If existe Then
                    ...
                    FacturacionProveedor.Limpiar_Datos_Comprobante_Proveedor(dgvFacturacionProveedor, txtSubTotal, txtDescuento, txtIVa, txtTotal, txtRazonSocial, txtDireccion, txtCelular, txtTelefono, txtCondIva, txtMail, txtLimiteCC, txtCodigoPostal, lblTipoComprobante, lblIdComprobante)
                    ...
                    FacturacionProveedor.Obtener_Datos_Comprobante_Proveedor(Numero_Condicion_IVA_Proveedor, Numero_Condicion_IVA_Empresa, dfielddefConstantes.FACTURA.ToString(), DatoTipoComprobante)
                    lblTipoComprobante.Text = DatoTipoComprobante.Descripcion   'tipoComprobante
    ...
    Private Sub btnBuscarProveedor_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscarProveedor.Click
        ...
        Try
            Proveedores.CompvariableProveedores = dfielddefConstantes.FACTURAPROVEEDOR.ToString()
            Me.Hide()
            Proveedor.Show()
        Catch ex As Exception
            MsgBox("Error:" & vbCrLf & ex.Message)
    ...
    Public Sub completar(ByVal CodigoP As Integer, ByVal fila As Integer, ByVal columna As Integer)
        ...
        Try
            ...
            articulo.recuperar_Datos_Producto_Producto_Lista_Precio_EmpresaArticulo(txtCodigoProveedor.Text.Trim(), Datos_Configuracion.Id_Lista_Precio, Convert.ToString(CodigoP), DatosEmpresa(0).Item("Id_Empresa"), datos)
            If datos.Rows.Count > 0 Then
                ...
                If DatoTipoComprobante.Descripcion = "FACTURA A" Then
                    dgvFacturacionProveedor.Item(enumerado.columnaTipoUnidad, fila).Value = datos.Rows(0).Item("Tipo_Unidad")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaCProdProv, fila).Value = datos.Rows(0).Item("Cod_Prod_Proveedor")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaCodArticulo, fila).Value = datos.Rows(0).Item("Id_Producto")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaDescripcionArt, fila).Value = datos.Rows(0).Item("Descripcion")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaUnidKilos, fila).Value = "0"
                    TasaIVa.recuperar_Datos(Convert.ToInt32(datos.Rows(0).Item("Id_Tasa_IVA")), DatosTasaIVA)
                    FacturacionProveedor.obtenerTasa(DatosTasaIVA.Rows(0).Item("Tasa"), ObtenerTasa)
                    dgvFacturacionProveedor.Item(enumerado.ColumnaPUnit, fila).Value = Format(Convert.ToDouble(Replace(datos.Rows(0).Item("Precio_Costo").ToString(), ",", ".")) / ObtenerTasa, "##,##0.00")
                ...
                ElseIf DatoTipoComprobante.Descripcion = "FACTURA B" Or DatoTipoComprobante.Descripcion = "FACTURA C" Then
                    dgvFacturacionProveedor.Item(enumerado.columnaTipoUnidad, fila).Value = datos.Rows(0).Item("Tipo_Unidad")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaCProdProv, fila).Value = datos.Rows(0).Item("Cod_Prod_Proveedor")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaCodArticulo, fila).Value = datos.Rows(0).Item("Id_Producto")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaDescripcionArt, fila).Value = datos.Rows(0).Item("Descripcion")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaUnidKilos, fila).Value = "0"
                    dgvFacturacionProveedor.Item(enumerado.ColumnaPUnit, fila).Value = datos.Rows(0).Item("Precio_Costo")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaImporte, fila).Value = "0"
                End If
            Else
                ...
                If articulo.CompId_Articulo <> "" Then
                    ...
                    dgvFacturacionProveedor.Item(enumerado.ColumnaCodArticulo, fila).Value = articulo.CompId_Articulo
                Else
                    If dgvFacturacionProveedor.Rows.Count > 0 Then
                        EliminarFila()
                    End If
        ...
        Try
            ...
            If datos.Rows.Count > 0 Then
                ...
                If DatoTipoComprobante.Descripcion = "FACTURA A" Then
                    ...
                    dgvFacturacionProveedor.Item(enumerado.ColumnaPUnit, fila).Value = Format(datos.Rows(0).Item("Precio_Costo").ToString() / ObtenerTasa, "##,##0.00")
                    dgvFacturacionProveedor.Item(enumerado.ColumnaImporte, fila).Value = "0"
            ...
            Else
                ...
                If articulo.CompDescripcion <> "" Then
                    dgvFacturacionProveedor.Item(enumerado.ColumnaDescripcionArt, fila).Value = articulo.CompDescripcion
    ...
    Private Sub dgvFacturacionProveedor_CellFormatting(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles dgvFacturacionProveedor.CellFormatting
        ...
        Try
            FacturacionProveedor.sumar_Importe(dgvFacturacionProveedor, totalImporte)
            ...
            If dgvFacturacionProveedor.Rows.Count > 0 Then
                If IsNumeric(Convert.ToString(dgvFacturacionProveedor.CurrentRow.Cells("CodigoArticulo").Value)) Then
                    ...
                    If DatoTipoComprobante.Descripcion = "FACTURA A" Then
                        ...
                        End If
                    Else
                        If DatoTipoComprobante.Descripcion = "FACTURA B" Or DatoTipoComprobante.Descripcion = "FACTURA C" Then
                            If txtDescuento.Text <> "" Then
    ...
    Private Sub ToolStripButtonRegistrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButtonRegistrar.Click
        ...
        If txtCodigoProveedor.Text <> "" Then
            If tbPuntoVenta.Text <> "" And tbNumeroComprobante.Text <> "" Then
                ...
                FacturacionProveedor.se_Cargo(tbPuntoVenta.Text.Trim(), tbNumeroComprobante.Text.Trim(), lblIdComprobante.Text.Trim(), txtCodigoProveedor.Text, existe)
                If Not existe Then
                    ...
                    If Convert.ToDouble(txtSubTotal.Text) > 0 Then
                        ...
                        FacturacionProveedor_Enc_estructura(1).NroPuesto = Session.Session.NroPuesto
                        'consulta = "select Id_Condicion_IVA from " + dfielddefConstantes.Condicion_Frente_Al_IVA.ToString() + " where Condicion_Frente_Al_IVA.Descripcion= '" & (txtCondIva.Text) & "' "
                        ...
                        FacturacionProveedor.Obtener_Datos_Comprobante_Cliente(Numero_Condicion_IVA_Cliente.Id_Condicion_IVA, Numero_Condicion_IVA_Empresa, DatoTipoComprobante)
                        ...
                        While i <= dgvFacturacionProveedor.Rows.Count
                            If dgvFacturacionProveedor.Rows(i - 1).Cells("CodigoArticulo").Value <> "" Then
                                ...
                                FacturacionProveedor_Cuerpo_estructura(i).NroPuesto = Session.Session.NroPuesto
                                ...
                                Articulo.ObtenerProductos(FacturacionProveedor_Cuerpo_estructura(i).Numero_Articulo, dtArticulos)
                        ...
                        End While
                        Try
                            Dim FPP As New FormasDePagoProveedor(FacturacionProveedor_Enc_estructura, FacturacionProveedor_Cuerpo_estructura, Articulos_Estructura)
                            formaPago.Compvariable = dfielddefConstantes.FACTURAPROVEEDOR.ToString()

                            FPP.ShowDialog()
                            If formaPago.Compvariable = dfielddefConstantes.Si.ToString() Then
                                LimpiarEstructuras()
                                FacturacionProveedor.Limpiar_Datos_Comprobante(dgvFacturacionProveedor, txtSubTotal, txtDescuento, txtIVa, txtTotal, txtRazonSocial, txtDireccion, txtCelular, txtTelefono, txtCondIva, txtMail, txtLimiteCC, tbPuntoVenta, tbNumeroComprobante, lblTipoComprobante, lblIdComprobante)
                                txtCodigoProveedor.Text = ""
                                lblCodN.Visible = False
                            Else
