...
Public Class Clientes
    ...
    Private Sub dgvClientes_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles dgvClientes.Click
        ...
        Try
            ...
            ClientesAltas.codigo_Cliente = id_Clientes
            ClientesAltas.NombreCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Nombre).Value.ToString()
            ClientesAltas.ApellidoCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Apellido).Value.ToString()
            ClientesAltas.calleCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Calle).Value.ToString()
            ClientesAltas.pisoCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Piso).Value.ToString()
            ClientesAltas.numeroCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Id_Cliente).Value.ToString()
            ClientesAltas.saldoCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Saldo_CC).Value.ToString()
            Cliente.Descomponer_CUIT_Cliente(dgvClientes.CurrentRow.Cells(dfielddefCliente.CUIT).Value.ToString(), cuit1, cuit2, cuit3)
            ClientesAltas.cuitCliente1.Text = cuit1
            ClientesAltas.cuitCliente2.Text = cuit2
            ClientesAltas.cuitCliente3.Text = cuit3

            ClientesAltas.provinciaCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Provincia).Value.ToString()
            ClientesAltas.telefonoCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Telefono).Value.ToString()
            ClientesAltas.celularCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Celular).Value.ToString()
            ClientesAltas.EMailCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.E_Mail).Value.ToString()
            ClientesAltas.codigoPostalCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Codigo_Postal).Value.ToString()
            ClientesAltas.responsabiliadIVACliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Responsabilidad_IVA).Value.ToString()
            ClientesAltas.localidadCliente.Text = dgvClientes.CurrentRow.Cells(dfielddefCliente.Localidad).Value.ToString()

            ClientesAltas.cbInhabilitar.Checked = dgvClientes.CurrentRow.Cells(dfielddefCliente.INHABILITAR).Value.ToString()
    ...
    Private Sub ToolStripEnviarCliente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripEnviarCliente.Click
        ...
        Dim Facturacion As New Vista.Facturacion()
        For x As Integer = pogressbarClientes.Minimum To pogressbarClientes.Maximum
            pogressbarClientes.Value = x
        Next
        For x As Integer = pogressbarClientes.Maximum To pogressbarClientes.Minimum Step -1
        ...
        If clientes.Compvariable = dfielddefconstante.NotaCredito.ToString() Then
            If (clientes.CompCodigo <> Nothing) Then
                NotaCredito.txtCodigoCliente.Text = clientes.CompCodigo.ToString()
                clientes.Compvariable = ""
            Else
                If (clientes.CompCodigo = Nothing) Then
                    NotaCredito.txtCodigoCliente.Text = Nothing
                End If
            End If
        End If
        If clientes.Compvariable = dfielddefconstante.NotaDebito.ToString() Then
            If (clientes.CompCodigo <> Nothing) Then
                NotaDebito.txtCodigoCliente.Text = clientes.CompCodigo.ToString()
                clientes.Compvariable = ""
            Else
                If (clientes.CompCodigo = Nothing) Then
                    NotaCredito.txtCodigoCliente.Text = Nothing
