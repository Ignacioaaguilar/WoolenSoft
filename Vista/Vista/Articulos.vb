...
Public Class Articulos
    ...
    Private Sub ToolStripModificarArticulo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripModificarArticulo.Click
        ...
        For x As Integer = ProgressBarArticulo.Maximum To ProgressBarArticulo.Minimum Step -1
            ProgressBarArticulo.Value = x
        Next
        ArticulosAltas.TBNumeroAsignado.Enabled = False
        ArticulosAltas.BTNGenerarCodigo.Enabled = False
        ArticulosAltas.CBRubro.Enabled = False
    ...
    Private Sub DGVArticulo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles DGVArticulo.Click
        ...
        Try
            ...
            ArticulosAltas.TBCodigoArticulo.Text = CStr(id_Articulo)
            ArticulosAltas.TBNumeroAsignado.Text = CStr(id_Articulo)

            ArticulosAltas.TBCodigoBarra.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Codigo_Barras).Value.ToString()
            ArticulosAltas.TBDescripcion.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Descripcion).Value.ToString()
            ...
            ArticulosAltas.TBStockMinimo.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Stock_Minimo).Value.ToString()
            ArticulosAltas.TBStock.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Stock).Value.ToString()

            ArticulosAltas.CBPesable.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Pesable).Value.ToString()
            ArticulosAltas.CBUnidades.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Tipo_Unidad).Value.ToString()
            ArticulosAltas.TBCantUnidCaja.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Cantidad_Unid_Caja).Value.ToString()
            ArticulosAltas.TBPesoUnidad.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Peso_Unidad).Value.ToString()

            ArticulosAltas.cbInhabiliarArticulo.Checked = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.INHABILITAR).Value.ToString()
            ArticulosAltas.tbCodProdProveedor.Text = DGVArticulo.CurrentRow.Cells(dfielddefArticulos.Cod_Prod_Proveedor).Value.ToString()
    ...
    Private Sub ToolStripEnviarArticulo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripEnviarArticulo.Click
        ...
        If articulo.Compvariable = dfielddefConstantes.ArticulosNotaCredito.ToString() Then
            If (articulo.Compvariable_Articulo <> Nothing) Then
                NotaCredito.txtBusquedaArticulo.Text = articulo.Compvariable_Articulo.ToString()
                articulo.Compvariable = String.Empty
            Else
                If (articulo.Compvariable_Articulo = Nothing) Then
                    NotaCredito.txtBusquedaArticulo.Text = Nothing
                    articulo.Compvariable = String.Empty
                End If
            End If
        End If
        If articulo.Compvariable = dfielddefConstantes.ArticulosNotaDebito.ToString() Then
            If (articulo.Compvariable_Articulo <> Nothing) Then
                NotaDebito.txtBusquedaArticulo.Text = articulo.Compvariable_Articulo.ToString()
                articulo.Compvariable = String.Empty
            Else
                If (articulo.Compvariable_Articulo = Nothing) Then
                    NotaDebito.txtBusquedaArticulo.Text = Nothing
                    articulo.Compvariable = String.Empty
                End If
            End If
        End If
        If articulo.Compvariable = dfielddefConstantes.ArticulosVentaRapida.ToString() Then
            If (articulo.Compvariable_Articulo <> Nothing) Then
                VentaRapida.txtBusquedaArticulo.Text = articulo.Compvariable_Articulo.ToString()
                articulo.Compvariable = String.Empty
            Else
                If (articulo.Compvariable_Articulo = Nothing) Then
                    VentaRapida.txtBusquedaArticulo.Text = Nothing
                End If
            End If
        End If
        Me.Close()
